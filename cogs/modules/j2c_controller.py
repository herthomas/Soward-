    ...
    try:
        ...
        async def get_view():
            ...
            lock_or_unlock_button = discord.ui.Button(
                ...
                emoji="🔓" if str(channel.permissions_for(channel.guild.default_role).connect) == 'False' else "🔒",
                row=4,
            )
            ...
            hide_or_unhide_button = discord.ui.Button(
                ...
                emoji="🙈" if str(channel.overwrites_for(channel.guild.default_role).view_channel) == 'False' else "👀",
                row=4,
            )
        ...
        async def lock_or_unlock_button_callback(interaction:discord.Interaction):
            try:
                ...
                if not channel_Data:
                    return await interaction.response.send_message("⛔ This channel is not a J2C channel.", ephemeral=True, delete_after=10)
                if channel_Data.get('owner_id') != interaction.user.id:
                    return await interaction.response.send_message("⛔ You are not the owner of this channel.", ephemeral=True, delete_after=10)
                if str(channel.permissions_for(channel.guild.default_role).connect) in ['True','None']:
                    ...
                    await interaction.response.send_message("✅ Channel has been locked.", ephemeral=True, delete_after=10)
                else:
                    await channel.set_permissions(channel.guild.default_role,connect=True)
                    await interaction.response.send_message("✅ Channel has been unlocked.", ephemeral=True, delete_after=10)
            except Exception as e:
                logger.error(f"Error in controller_module.lock_or_unlock_button_callback: {e}")
                await interaction.response.send_message(f"⚠️ An error occurred while locking/unlocking the channel: {e}", ephemeral=True, delete_after=10)
            try:
        ...
        async def hide_or_unhide_button_callback(interaction:discord.Interaction):
            try:
                ...
                if str(channel.overwrites_for(channel.guild.default_role).view_channel) in ['True','None']:
                    ...
                    await interaction.response.send_message("✅ Channel has been hidden.", ephemeral=True, delete_after=10)
                else:
                    await channel.set_permissions(channel.guild.default_role,view_channel=True)
                    await interaction.response.send_message("✅ Channel has been unhidden.", ephemeral=True, delete_after=10)
            except Exception as e:
                logger.error(f"Error in controller_module.hide_or_unhide_button_callback: {e}")
                await interaction.response.send_message(f"⚠️ An error occurred while hiding/unhiding the channel: {e}", ephemeral=True, delete_after=10)
            try:
                asyncio.create_task(update_channel())
            except:
                pass
        ...
        async def switch_region_select_callback(interaction:discord.Interaction):
            ...
            try:
                ...
                defer_message = await interaction.edit_original_response(content=f"✅ Region has been changed to `{new_region if new_region else 'Automatic'}`.",view=None)
                logger.info(f"✅ Channel region changed to {new_region if new_region else 'Automatic'}")
                try:
                    asyncio.create_task(update_channel())
                except:
                    pass
                ...
                defer_message = await interaction.edit_original_response(content=f"⚠️ An error occurred while changing the region: {e}",view=None)
                try:
                    asyncio.create_task(update_channel())
                except:
                    pass
            ...
            try:
                ...
                async def select_user_menu_callback(interaction:discord.Interaction):
                    ...
                    try:
                        ...
                        return await interaction.response.send_message("⚠️ User not found.", ephemeral=True, delete_after=10)
                    if new_owner.id == channel_Data.get('owner_id'):
                        return await interaction.response.send_message("⚠️ You are already the owner of this channel.", ephemeral=True, delete_after=10)

                    if new_owner.voice == None or new_owner.voice.channel != channel:
                        return await interaction.response.send_message("⚠️ The user is not in the channel.\nThis action can only be performed if the user is in the channel.", ephemeral=True, delete_after=10)
                    await interaction.response.defer()
                    try:
                        await on_voice_state_update.change_j2c_owner(bot,channel_Data,interaction.channel,new_owner)
                        await interaction.edit_original_response(content=f"✅ Ownership has been transferred to {new_owner.mention}.",view=None,embed=None)
                        logger.info(f"✅ Ownership of the channel has been transferred to {new_owner}")
                        await asyncio.sleep(10)
                        try:
                        ...
                        await interaction.edit_original_response(content=f"⚠️ An error occurred while transferring ownership: {e}",view=None)
                        await asyncio.sleep(10)
                        try:
                            await interaction.delete_original_message()
                        except:
            ...
            class ChangeUserLimit(discord.ui.Modal,title="Change User Limit"):
                ...
                async def on_submit(self,interaction:discord.Interaction):
                    ...
                    try:
                        ...
                        return await interaction.response.send_message("⚠️ User limit must be a number.", ephemeral=True, delete_after=10)
                    if new_user_limit < 0:
                        return await interaction.response.send_message("⚠️ User limit must be greater than or equal to `0`.", ephemeral=True, delete_after=10)
                    if new_user_limit == channel.user_limit:
                        return await interaction.response.send_message("⚠️ The new user limit is the same as the current user limit.", ephemeral=True, delete_after=10)
                    await interaction.response.defer(thinking=True,ephemeral=True)
                    try:
                        await channel.edit(user_limit=new_user_limit if new_user_limit != 0 else None)
                        defer_message = await interaction.edit_original_response(content=f"✅ User limit has been changed to `{new_user_limit if new_user_limit != 0 else 'Unlimited'}`.",view=None)
                        try:
                            await asyncio.create_task(update_channel())
                        except:
                            pass
                        logger.info(f"✅ Channel user limit changed to {new_user_limit if new_user_limit != 0 else 'Unlimited'}")
                        await asyncio.sleep(10)
                        try:
                            await defer_message.delete()
                        except:
                        ...
                        defer_message = await interaction.edit_original_response(content=f"⚠️ An error occurred while changing the user limit: {e}",view=None)
                        try:
                            await asyncio.create_task(update_channel())
                        except:
                            pass
            ...
            class ChangeBitrate(discord.ui.Modal,title="Change Bitrate"):
                ...
                async def on_submit(self,interaction:discord.Interaction):
                    ...
                    try:
                        ...
                        return await interaction.response.send_message("⚠️ Bitrate must be a number.", ephemeral=True, delete_after=10)
                    if new_bitrate_in_kbps < 8:
                        return await interaction.response.send_message("⚠️ Bitrate must be greater than `8kbps`.", ephemeral=True, delete_after=10)
                    
                    if new_bitrate_in_kbps > int(interaction.guild.bitrate_limit)/1000:
                        return await interaction.response.send_message(f"⚠️ Bitrate cannot be greater than `{int(interaction.guild.bitrate_limit)/1000}kbps`.", ephemeral=True, delete_after=10)
                    if new_bitrate_in_kbps == channel.bitrate/1000:
                        return await interaction.response.send_message("⚠️ The new bitrate is the same as the current bitrate.", ephemeral=True, delete_after=10)
                    await interaction.response.defer(thinking=True,ephemeral=True)
                    try:
                        await channel.edit(bitrate=new_bitrate_in_kbps*1000)
                        defer_message = await interaction.edit_original_response(content=f"✅ Bitrate has been changed to `{new_bitrate_in_kbps}kbps`.",view=None)
                        try:
                            await asyncio.create_task(update_channel())
                        except:
                            pass
                        logger.info(f"✅ Channel bitrate changed to {new_bitrate_in_kbps}kbps")
                        await asyncio.sleep(10)
                        try:
                            await defer_message.delete()
                        except:
                        ...
                        defer_message = await interaction.edit_original_response(content=f"⚠️ An error occurred while changing the bitrate: {e}",view=None)
                        try:
                            await asyncio.create_task(update_channel())
                        except:
                            pass
            ...
            class ChangeChannelName(discord.ui.Modal,title="Change Channel Name"):
                ...
                async def on_submit(self,interaction:discord.Interaction):
                    ...
                    if new_name == channel.name:
                        return await interaction.response.send_message("⛔ The new name is the same as the current name.", ephemeral=True, delete_after=10)
                    await interaction.response.defer(thinking=True,ephemeral=True)
                    try:
                        await channel.edit(name=new_name)
                        defer_message = await interaction.edit_original_response(content="✅ Channel name has been changed.",view=None)
                        try:
                            await asyncio.create_task(update_channel())
                        except:
                            pass
                        ...
                        defer_message = await interaction.edit_original_response(content=f"⚠️ An error occurred while changing the channel name: {e}",view=None)
                        try:
                            await asyncio.create_task(update_channel())
                        except:
                            pass
